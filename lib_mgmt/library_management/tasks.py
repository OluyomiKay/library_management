# -*- coding: utf-8 -*-
# Copyright (c) 2015, Yomi and contributors
# For license information, please see license.txt

from __future__ import unicode_literals
import frappe
from frappe.utils.data import date_diff, nowdate, formatdate, add_days
...
def daily():
    ...
    for member, items in overdue.iteritems():
        ...
        frappe.sendmail(recipients=[frappe.db.get_value("Library Member", member, "email")],
            sender="librarian@example.com", subject="Library Articles Overdue", msg=content, bulk=True)


# recipient = frappe.db.get_value("Library Member", member, "email")
#		frappe.sendmail(recipients=[recipient],
#			sender="librarian@example.com",
#			subject="Library Articles Overdue", content=content, bulk=True)

def get_overdue(loan_period):
    # checking for overdue articles
    today = nowdate()

    overdue_by_member = {}
    articles_transacted = []

    for d in frappe.db.sql('''select name, article, article_name, library_member, member_name from `tabLibrary Transaction` order by transaction_date DESC, modified DESC''', as_dict=1):
